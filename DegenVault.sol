...
// Degen Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract DegenVault is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accDegenPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accDegenPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accDegenPerShare; // Accumulated KOREs per share, times 1e12. See below.
    ...
    function pendingDegen(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accDegenPerShare = pool.accDegenPerShare;

        return user.amount.mul(accDegenPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    uint256 private degenBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = degen.balanceOf(address(this)).sub(degenBalance);

        if(newRewards > 0) {
            degenBalance = degen.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 degenRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        degenRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 degenRewardFee = degenRewardWhole.mul(DEV_FEE).div(10000);
        uint256 degenRewardToDistribute = degenRewardWhole.sub(degenRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(degenRewardFee);

        pool.accDegenPerShare = pool.accDegenPerShare.add(
            degenRewardToDistribute.mul(1e12).div(tokenSupply)
        );

    }

    // Deposit  tokens to DegenVault for KORE allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accDegenPerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accDegenPerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from DegenVault.
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.accDegenPerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safeDegenTransfer(from, pending);
        }
    ...
    // Safe degen transfer function, just in case if rounding error causes pool to not have enough KOREs.
    function safeDegenTransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 degenBal = degen.balanceOf(address(this));
        if (_amount > degenBal) {
            console.log("transfering out for to person:", _amount);
            console.log("Balance of this address is :", degenBal);

            degen.transfer(_to, degenBal);
            degenBalance = degen.balanceOf(address(this));

        } else {
            degen.transfer(_to, _amount);
            degenBalance = degen.balanceOf(address(this));
        ...
        if(pending_DEV_rewards > 0) {
            ...
            safeDegenTransfer(devaddr, devSend);
        }
